...
#' @usage gif(dta, nt = 100L, nss = NULL, threshold = 0.95)
#' @param dta A wide-format \code{data.frame} object with records (stored by row).
#' @param nt Number of generalized isolation trees to build to form the forest. By default, it is set to \code{100}.
#' @param nss Number of subsamples used to build a single generalized isolation tree.
...
gif <- function(dta, nt = 100L, nss = NULL, threshold = 0.95) {
	...
	dta <- lapply(dta, function(x) {
	  if (is.numeric(x)) {
	    ...
	    x[is.na(x)] <- "_NA_HRTnomaly_"
	  } else if (is.factor(x)) {
		levels(x) <- c(levels(x), "_NA_HRTnomaly_")
		x[is.na(x)] <- "_NA_HRTnomaly_"
	  }
      return(x)
    })
	whchr <- sapply(dta, is.character)
	whftc <- sapply(dta, is.factor)
	whchr <- whchr | whftc
	dtchr <- data.frame(lapply(dta[whchr], as.factor))
	dtchr <- if (prod(dim(dtchr)) > 0) model.matrix(~ . + 0, data = dtchr) else c()
	dtnum <- data.frame(dta[!whchr])
